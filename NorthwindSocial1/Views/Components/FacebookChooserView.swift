...
struct FacebookChooserView: View {
    ...
    private var introduction: some View {
        VStack(alignment: .leading, spacing: 28) {
            Spacer()
            Image("Background").resizable().scaledToFit().frame(width: 64, height: 64)
            Text("All the chrome in one bar.").font(.system(size: 34, weight: .bold))
            Text("Tabs and tools stay out of the way, so Facebook gets the whole window.")
                .font(.title3).foregroundStyle(.secondary).lineSpacing(8).frame(maxWidth: 490, alignment: .leading)
